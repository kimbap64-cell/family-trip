...
EXCL_RE = re.compile(r"부속시설|스터디|독서실|학원|병원|약국|부동산|편의점|마트|미용|세탁|은행|주유소|주차장|공원|체육|헬스|교회|어린이집|유치원")
...
PAT = {
    "kids": {
        ...
        "유모차": r"유모차",
        "체험·동물": r"(먹이\s*주|먹이주기|(?<!반려)(?<!애완)동물|체험(?!단)|수확|물놀이|썰매|만들기|승마|레일\s*바이크|미끄럼틀)",
        "캠핑 키즈시설": r"(방방|트램펄린|에어바운스|수영장|풀장|워터\s*슬라이드|모래\s*놀이|계곡|키즈\s*(존|놀이|사이트)|물놀이장|아이들\s*놀)",
    },
    "b_pos": {
        "입식/테이블": r"(입식|테이블석|테이블\s*좌석|테이블\s*자리|의자\s*(좌석|자리|석)|홀\s*좌석|(책상|테이블)\s*(으로|로)\s*되어)",
        ...
        "캠핑 편의(평탄·주차·화장실)": r"(사이트\s*(가\s*)?(평평|평탄|넓)|파쇄석|데크\s*사이트|차량\s*(진입|옆|바로)|주차\s*(가\s*)?(사이트|옆|바로)|(화장실|샤워장|개수대)\s*(이\s*)?(가까|깨끗|청결|온수|따뜻))",
        "이동수단": r"(모노레일|전동\s*카트|셔틀\s*(버스|운행|이용)|트램(?!펄|플|폴)|꼬마\s*열차|골프\s*카트)",  # '트램플린/트램펄린' 오판 방지
        "쉼터·벤치·그늘": r"(벤치|쉼터|휴게\s*(소|공간|실)|정자|앉아\s*쉴|쉬어\s*갈\s*수|쉬어갈\s*수|쉴\s*수\s*있)",
        "평탄·데크길": r"(평탄|완만한|무장애\s*(길|데크|산책|나눔)|데크\s*(길|로드)|포장\s*(된\s*)?(길|도로)|유모차\s*(도\s*)?(이동|다니|끌))",
    },
    "b_neg": {
        ...
        "긴 보행": r"(많이\s*걸어야|한참\s*걸어야|오래\s*걸어야|걷기\s*(가\s*)?(힘들|어렵|부담)|체력\s*(이\s*)?(필요|소모|많이)|왕복\s*\d+\s*(분|시간)\s*(정도\s*)?(걸|소요)|편도\s*\d+\s*(분|시간)\s*(정도\s*)?(걸어|걷|소요))",
    },
    "plan": {
        "사전 예약": r"(사전\s*예약\s*(필수|필요|제|만)|예약\s*(필수|제로|해야)|예약\s*없이\s*(입장|이용)\s*(은\s*)?불가|예약(을)?\s*(하지\s*않으면|안\s*하면)\s*(입장|이용)|예약\s*오픈|예약\s*경쟁|300대\s*이하만\s*예약)",
        "휴관·휴무일": r"((매주|매월)\s*(첫째|둘째|셋째|넷째|\d)?\s*[월화수목금토일]요일\s*(정기\s*)?(휴관|휴무|휴장|휴원|쉬)|휴관\s*[:：]?\s*매주\s*[월화수목금토일]|[월화수목금토일]요일\s*(정기\s*)?(휴관|휴무|휴장|휴원))",
    },
    # 캠핑 전용 평가 요소: 개별 화장실·샤워실 또는 매우 청결한 화장실·샤워실 (사용자 기준, 2026-09-26)
    "bath": {
        "개별 화장실·샤워실": r"((개별|전용|독립|프라이빗|객실\s*내|사이트\s*(내|안|별)|텐트\s*안|카라반\s*(내|안)|룸\s*내)\s*(에\s*)?(화장실|샤워실|샤워장|샤워\s*부스|욕실|샤워\s*시설)|(화장실|샤워실|샤워장|욕실)\s*(이|가|은|도)?\s*(개별|전용|독립|딸려|딸린|객실\s*안|내부에\s*있))",
        "화장실·샤워실 청결": r"((화장실|샤워실|샤워장|샤워\s*시설|세면대|위생\s*시설|위생)\s*(이|가|은|도|시설\s*(이|도)?)?\s*(정말|너무|아주|매우|굉장히|엄청|늘|항상|진짜|완전)?\s*(깨끗|청결|쾌적|깔끔|최신|리모델링|새것)|(깨끗|청결|깔끔|쾌적)\s*(한|하고|해서)\s*(화장실|샤워실|샤워장|샤워\s*시설|세면대))",
        "온수 잘 나옴": r"((온수|따뜻한\s*물)\s*(가|는|도)?\s*(잘|콸콸|풍부|충분|넉넉|무제한)\s*(나|나와|나오|나왔))",
        "화장실·샤워실 아쉬움": r"((화장실|샤워실|샤워장|샤워\s*시설)\s*(이|가|은|도)?\s*(너무\s*|좀\s*|많이\s*|꽤\s*)?(멀|불결|더럽|지저분|냄새|낡|불편|아쉽|아쉬|부족|협소|좁|줄\s*서|대기))",
    },
    "closure": {
    ...
    for g, d in COMPILED.items():
        for label, rx in d.items():
            ...
            for m in rx.finditer(text):
                w = 24 if g == "plan" else 45  # 예약·휴관 안내는 짧게 (핵심 문구 주변만)
                ...
                if label == "계단" and re.search(r"엘리베이터|엘베", win):
                    continue
                # '바닥에 앉을 공간이 부족/테이블·책상으로 되어 있다' 는 좌식이 아니라 입식이라는 뜻 (사노리숲 오판 사례)
                if label == "좌식" and re.search(r"(부족|없|테이블|책상|의자|입식|아니)", win):
                    continue
                # 탑승(모노레일·카트)을 설명하는 오르막/계단 언급은 주의 근거가 아님 ('오르막길을 타고 이동')
                if label == "계단" and re.search(r"(타고|탑승|모노레일|카트|셔틀)", win):
                    continue
                # '걷기 힘든 어른들도 좋아요' 처럼 긍정 문맥의 보행 언급은 부정 근거가 아님
                if label == "긴 보행" and re.search(r"(좋아요|좋습니다|좋은|추천|편하|편해|수월|않아도|없이|모시고)", win):
                    continue
                # 화장실·샤워실 '아쉬움'은 부정어가 뒤에 붙으면('냄새 안 나요','멀지 않아요','불편함 없이') 근거가 아님
                if label == "화장실·샤워실 아쉬움" and re.search(r"(않|안\s*(나|났|남|불편)|없|전혀|하나도|걱정\s*마|괜찮)", text[m.end(): m.end() + 10]):
                    continue
                # '여성 전용 샤워장' 같은 성별 전용은 개별(가족 전용) 화장실·샤워실이 아님 (힐링별밤수목원 오판 사례)
                if label == "개별 화장실·샤워실" and re.search(r"(여성|남성|여자|남자|남녀|장애인|공용)\s*(전용|화장실)", text[max(0, m.start() - 6): m.end() + 2]):
                    continue
                # '개별화장실 느낌', '칸이라 …' 는 칸막이 공용 시설을 비유한 표현이지 가족 전용이 아님 (명랑가출 오판 사례)
                if label == "개별 화장실·샤워실" and re.search(r"(느낌|처럼|칸이라|칸으로|칸막이|같은)", text[max(0, m.start() - 40): m.end() + 14]):
                    continue
                # '깨끗' 도 앞뒤에 부정('깨끗하지 않', '깨끗한 편은 아니')이면 근거가 아님
                if label == "화장실·샤워실 청결" and re.search(r"(않|아니|못\s*했|그다지|별로|아쉽)", text[m.end(): m.end() + 8]):
                    continue
